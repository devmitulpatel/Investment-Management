...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.kisanVikarPatraForUser.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.kisan-vikar-patra-for-users.update", [$kisanVikarPatraForUser->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="date_of_purchase">{{ trans('cruds.kisanVikarPatraForUser.fields.date_of_purchase') }}</label>
                <input class="form-control date {{ $errors->has('date_of_purchase') ? 'is-invalid' : '' }}" type="text" name="date_of_purchase" id="date_of_purchase" value="{{ old('date_of_purchase', $kisanVikarPatraForUser->date_of_purchase) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_of_maturity') ? 'is-invalid' : '' }}" type="text" name="date_of_maturity" id="date_of_maturity" value="{{ old('date_of_maturity', $kisanVikarPatraForUser->date_of_maturity) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('certificate_no') ? 'is-invalid' : '' }}" type="text" name="certificate_no" id="certificate_no" value="{{ old('certificate_no', $kisanVikarPatraForUser->certificate_no) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_paid') ? 'is-invalid' : '' }}" type="number" name="amount_paid" id="amount_paid" value="{{ old('amount_paid', $kisanVikarPatraForUser->amount_paid) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_received') ? 'is-invalid' : '' }}" type="number" name="amount_received" id="amount_received" value="{{ old('amount_received', $kisanVikarPatraForUser->amount_received) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('holders') ? 'is-invalid' : '' }}" name="holders[]" id="holders" multiple>
                    @foreach($holders as $id => $holder)
                        <option value="{{ $id }}" {{ (in_array($id, old('holders', [])) || $kisanVikarPatraForUser->holders->contains($id)) ? 'selected' : '' }}>{{ $holder }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nominees') ? 'is-invalid' : '' }}" name="nominees[]" id="nominees" multiple>
                    @foreach($nominees as $id => $nominees)
                        <option value="{{ $id }}" {{ (in_array($id, old('nominees', [])) || $kisanVikarPatraForUser->nominees->contains($id)) ? 'selected' : '' }}>{{ $nominees }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('purchase_from') ? 'is-invalid' : '' }}" name="purchase_from" id="purchase_from">{{ old('purchase_from', $kisanVikarPatraForUser->purchase_from) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ref_contact_name') ? 'is-invalid' : '' }}" type="text" name="ref_contact_name" id="ref_contact_name" value="{{ old('ref_contact_name', $kisanVikarPatraForUser->ref_contact_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ref_contact_no') ? 'is-invalid' : '' }}" type="number" name="ref_contact_no" id="ref_contact_no" value="{{ old('ref_contact_no', $kisanVikarPatraForUser->ref_contact_no) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\KisanVikarPatraForUser::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $kisanVikarPatraForUser->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
